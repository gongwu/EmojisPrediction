...
class Task(object):
    ...
    def __init__(self, init=False, FLAGS=None):
        self.FLAGS = FLAGS
        self.train_file = config.train_file_final
        self.dev_file = config.dev_file_final
        # 测试集之后添加
        ...
        self.train_data = Dataset(examples_train, self.word_vocab, self.char_vocab, self.ner_vocab, self.pos_vocab, self.rf_vocab, self.max_sent_len, self.max_word_len, self.num_class )
        self.dev_data = Dataset(examples_dev, self.word_vocab, self.char_vocab, self.ner_vocab, self.pos_vocab, self.rf_vocab, self.max_sent_len, self.max_word_len, self.num_class)
        if self.test_file:
            self.test_data = Dataset(examples_test, self.word_vocab, self.char_vocab, self.ner_vocab, self.pos_vocab, self.rf_vocab, self.max_sent_len, self.max_word_len, self.num_class)
